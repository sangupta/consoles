...
/**
 * <code>TextMouseEvents</code> are fired in response to mouse clicks on
 * <code>TextWindows</code> which feature mouse support. The coordinates of the
 * mouse click indicate the character which was clicked (not the actual pixel
 * coordinates), and there is also a bias which indicates whether the left or
 * right side of the character was clicked.
 *
 * @status.stable
 * @see TextMouseEvent
 * @see TextMouseAdapter
 * @author Ethan Royael Nicholas (
 *         <a href="mailto:ethan@ethannicholas.com">ethan@ethannicholas.com</a>)
 */
public class TextMouseEvent extends MouseEvent {
    ...
    /**
	 * Constructs a new <code>TextMouseEvent</code> with the specified source
	 ...
	 * 
	 * @param modifiers
	 *            the modifier keys down during event (e.g. shift, ctrl, alt,
	 *            meta) Either extended _DOWN_MASK or old _MASK modifiers should
	 *            be used, but both models should not be mixed in one event. Use
	 *            of the extended modifiers is preferred.
	 * 
	 * @param x
	 ...
	 */
    public TextMouseEvent(Component source, int id, long when, int modifiers, int x, int y, int clickCount, boolean popupTrigger, int bias) {
